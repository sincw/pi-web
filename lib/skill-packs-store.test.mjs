...
test("readConfig returns an empty config when the file is missing", () => {
  const root = mkdtempSync(join(tmpdir(), "pi-web-packs-"));
  try {
    ...
    writeConfig(
      {
        ...
        packs: [{ id: "p1", name: "PPT", description: "d", skills: [{ skillKey: "slides", contentHash: "h" }] }],
      },
      { configPath: path },
    );
...
test("writeConfig is atomic when nested dir is missing", () => {
  ...
  try {
    ...
    writeConfig({ version: 1, libraryRoot: null, packs: [] }, { configPath: path });
    assert.ok(existsSync(path));
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
...
test("createPack adds a pack with a generated id", () => {
  ...
  const base = { version: 1, libraryRoot: null, packs: [] };
...
test("updatePack edits name/description/skills and ignores unknown id", () => {
  const base = {
    version: 1,
    libraryRoot: null,
    packs: [{ id: "p1", name: "old", description: "", skills: [] }],
  };
...
test("deletePack removes a pack but nothing else", () => {
  const base = {
    ...
    libraryRoot: null,
    packs: [
      { id: "p1", name: "a", description: "", skills: [] },
      { id: "p2", name: "b", description: "", skills: [] },
    ],
  };
...
test("findPacksReferencingSkillKey lists packs referencing a skillKey (case-insensitive)", () => {
  const base = {
    ...
    libraryRoot: null,
    packs: [
      { id: "a", name: "A", description: "", skills: [{ skillKey: "slides", contentHash: "h" }] },
      { id: "b", name: "B", description: "", skills: [{ skillKey: "outline", contentHash: "h" }] },
      { id: "c", name: "C", description: "", skills: [{ skillKey: "Slides", contentHash: "h" }] },
    ],
  };
...
test("getPackById returns the pack or null", () => {
  const base = { version: 1, libraryRoot: null, packs: [{ id: "p1", name: "x", description: "", skills: [] }] };
