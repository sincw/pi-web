...
function pack(id, name, refs) {
  return { id, name, description: "", skills: refs, mcpServers: [] };
}
...
test("readWorkspaceState returns an empty state when the file is missing", () => {
  const root = mkdtempSync(join(tmpdir(), "pivot-ui-ws-"));
  try {
    const state = readWorkspaceState({ cwd: root });
    assert.equal(state.version, 2);
    assert.equal(state.revision, 0);
    assert.deepEqual(state.mcp, { disabledServerKeys: [], managedServers: {} });
      ...
      appliedPacks: [
        {
          packId: "p1",
          status: "full",
          receipt: { appliedAt: "t", installed: [ref("slides", "h")], mcpServers: [] },
        },
      ],
      skippedConflicts: [{ packId: "p1", skillKey: "outline", reason: "same_name_exists" }],
      mcp: { disabledServerKeys: [], managedServers: {} },
    };
...
test("readWorkspaceState migrates v1 receipts without MCP ownership", () => {
  const root = mkdtempSync(join(tmpdir(), "pivot-ui-ws-"));
  try {
    const statePath = getWorkspaceStatePath(root);
    mkdirSync(join(root, ".pi"), { recursive: true });
    writeFileSync(statePath, JSON.stringify({
      version: 1,
      appliedPacks: [{ packId: "p1", status: "full", receipt: { appliedAt: "t", installed: [] } }],
      skippedConflicts: [],
    }));
    const state = readWorkspaceState({ cwd: root });
    assert.equal(state.version, 2);
    assert.equal(state.revision, 0);
    assert.deepEqual(state.appliedPacks[0].receipt.mcpServers, []);
    assert.deepEqual(state.mcp, { disabledServerKeys: [], managedServers: {} });
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
...
test("upsertAppliedPack inserts then updates by packId (pure)", () => {
  const base = { version: 2, revision: 0, appliedPacks: [], skippedConflicts: [], mcp: { disabledServerKeys: [], managedServers: {} } };
  const applied = {
    packId: "p1",
    status: "full",
    receipt: { appliedAt: "t", installed: [ref("slides", "h")], mcpServers: [] },
...
test("getAppliedPack looks up by packId", () => {
  const state = {
    version: 2,
    revision: 0,
    appliedPacks: [{ packId: "p1", status: "full", receipt: { appliedAt: "", installed: [], mcpServers: [] } }],
    skippedConflicts: [],
    mcp: { disabledServerKeys: [], managedServers: {} },
  };
