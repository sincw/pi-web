...
| 同名冲突策略 | 「只装不删」，同名跳过保住手改 | 以工作空间收据识别 pi-web 写入的条目；未知或已手改的条目绝不自动覆盖 |
| 撤销"应用" | **不可逆**：删文件可能丢用户改动 → 选择"只摘标签不删文件" | 重新计算剩余 Pack 的并集，仅移除不再需要且仍由 Pack 托管的条目 |
| 团队共享 | `.pi/skills/` 进 git 共享 | `.mcp.json`（团队）/ `.pi/mcp.json`（pi 专属）进 git 共享 |
| 个人临时试用 | 不适用 | 靠 apply + 撤销；secret 靠 `bearerTokenEnv`；多会话隔离靠不同 cwd |

**结论**：统一入口和概念模型，**不强行统一应用动作**。Skill 应用是"单向复制"（lossy to reverse），MCP 应用是"双向开关"（reversible）。产品文案必须讲清这个区别。
...
5. 作为一个要接 OAuth 型 MCP server（如 Linear、Figma）的用户，我希望在 pi-web 里发起授权流程：点「连接」→ 拿到授权 URL → 在我本地浏览器授权 → 把回调 URL 粘回 pi-web → 完成。pi 进程跑在服务器上无法自动开我的浏览器，所以这一步必须是复制粘贴式。
...
9. 作为一个想给某 server 把个别工具提到 Pi 一等工具（direct tools）的用户，我希望看到它只能配置在全局 `~/.pi/agent/mcp.json` 并重启 pi-web，避免误以为 Pack 已让它生效。

10. 作为一个想分享环境的用户，我希望导出 Pack 为 JSON 分享给同事，对方粘贴/上传一键导入。

11. 作为一个团队仓库协作者，我希望项目里的 `.mcp.json`（团队共享）始终生效，不被我的 Pack 覆盖。

12. 作为一个刚装好 pi 的用户，我希望 pi-web 提示我「adapter 已装、全局默认 MCP 在 `~/.pi/agent/mcp.json`、还没有自定义 Pack 里的 MCP server」，第一次打开管理有引导。
...
| **托管条目** | pi-web 写入 `.pi/mcp.json` 后，在项目收据中记录了基线哈希的 `mcpServers` 条目；只有它可被后续重整替换或移除。 |
| **外来条目** | `.pi/mcp.json` 中不在收据内，或与收据基线不同的条目；它可能来自用户、其他工具或一次手动编辑，pi-web 始终原样保留。 |
...
| 多包协作 | 并集去重 | 并集去重 |
| 撤销 | 只摘标签不删文件 | 移除标签后重整；仅删除不再属于任何 Pack 且未被用户修改的托管 key |
...
- `managedServers` 是**项目文件中的基线**，不是第二份配置。重整前重新计算当前条目的哈希：相同才仍可由 pi-web 管理；不同即降级为外来条目并保留。
- `disabledServerKeys` 仅对当前 Pack 并集中的 pi-web 托管 server 有效。某 key 不再在并集中时自动清理。
...
前端按下表降级，后端所有会改动 MCP 状态的路由仍必须二次检查；未就绪时返回 `412 MCP_ADAPTER_REQUIRED`，避免绕过 UI 写出无效的“已应用”收据。

| 位置 / 动作 | adapter 未就绪时的行为 |
|---|---|
| `Packs` 入口和纯 Skill Pack | 保持可用，避免 MCP 扩展阻塞既有技能管理 |
| Pack 编辑器的 MCP servers 区 | 灰显，只展示前置条件和安装/启用按钮；技能区照常可编辑 |
| MCP 库录入、导入、编辑、删除 | 禁用，防止创建当前无法使用的连接定义 |
| 包含 MCP 引用的 Apply / 移除标签 / Toggle | 禁用；纯 Skill Pack 的 Apply / 移除不受影响 |
| `SkillsConfig` 的 MCP 区与项目有效环境预览 | 不计算“有效环境”，显示未就绪状态和 CTA；可保留 `.mcp.json` / `.pi/mcp.json` 的只读文件查看 |
| 当前会话 MCP 重载 | 禁用 |

这不是权限模型。用户仍可自行编辑 JSON；Guard 的目的只是保证 pi-web 不会把一个未被 adapter 消费的配置标成“已应用”。
...
- 同名同哈希跳过；同名不同哈希冲突跳过并记 `skippedConflicts`。
- 移除标签不删文件。
...
1. 读取工作空间收据，将“当前标签 + 本次增删后的标签”解析为 Pack 列表；对于已删除的 Pack 使用其 `receipt.mcpServers` 回退，并提示用户清理无效标签。
...
4. 从期望并集中排除 `disabledServerKeys`，得到需要启用的 server 集合。
5. 读取现有 `.pi/mcp.json`，对每个收据中的托管 key 校验当前哈希。哈希不同则降级为外来条目，保留原样，不再替换或删除它。
6. 在仍受托管的 key 中：期望集合内的写入库中定义；不在期望集合内的删除。新 key 仅在目标位置不存在时写入并记录基线哈希。
7. 外来 key、已降级 key、`settings`、`imports` 和未知顶层字段一律原样保留；最后以临时文件 + 原子 rename 写回，成功后更新收据。
...
- **会话内“重载 MCP”** = 先在项目视图完成 Pack 变更，再执行 `inner.reload()`，带确认对话框。
...
**关键约束**：adapter 的 `ServerEntry`（`pi-mcp-adapter@2.11.0` `types.ts:284`）**没有 `disabled` 字段** —— 进了 `mcpServers` 就是激活。pi-web 不能靠给条目加 flag，只能**从工作空间成品文件移除条目 + 自己记一份禁用清单**。
...
**适用范围**：只对 pi-web 自己写入 `.pi/mcp.json` 的 server 生效。来自团队 `.mcp.json`、`~/.config/mcp/mcp.json`、`~/.pi/agent/mcp.json` 的 server 只读展示，Toggle 锁状 + tooltip 指路编辑哪个文件。
...
- adapter 未就绪时不隐藏整个入口：Pack 仍用于管理 skills；MCP 子区按 §6.4 灰显并在原位置给出安装/启用动作，避免用户误以为功能消失或其数据丢失。
...
- workspace 已应用 Pack 标签上点 `×` 移除：从完整标签集合中移除后重整。skills 采用已落地的并集卸载规则；MCP 仅删除不再在并集中且仍由 pi-web 托管的条目。
...
☁ team-shared-git    stdio · lazy      [Toggle 锁]
  来源: team · .mcp.json （不归 pi-web 管，请编辑该文件）
```

- Toggle = 启用/禁用（§7.3 机制）。
- 来源徽标：pack applied / team / global / adapter-global。
- 不归 pi-web 管的：Toggle 锁状 + tooltip。
- 排序：启用在上、禁用在中、外来只读末尾。
- 应用默认启用。
...
| `/api/workspace-skill-packs/preview` | 扩展为针对“现有标签 + 本次增删”生成 skill + MCP 的完整并集计划 |
| `/api/workspace-skill-packs/apply` | 执行已确认计划；收据版本校验失败则拒绝，避免并发覆盖 |
| `/api/workspace-mcp/toggle` | PATCH `{ cwd, serverKey, enabled }`：启用/禁用项目工作空间 MCP |
| `/api/agent/[id]` | v1.1 新增 `type: "reload_mcp"`：在项目配置已变更后重载当前会话；不接收单个 `packId` |
...
`/api/skill-library/mcp-servers*`、`/api/workspace-mcp/toggle`、包含 MCP 引用的 workspace preview/apply，以及 `reload_mcp` 都先检查 `/api/mcp/status` 等价的服务端 guard；未就绪统一返回 `412 MCP_ADAPTER_REQUIRED`。`/api/skill-packs*` 对纯 skill 字段仍可正常工作。

`lib/mcp-pack-apply.ts` 负责：写 `.pi/mcp.json`、撤销、冲突、toggle。

`lib/rpc-manager.ts` 的 `startRpcSession` **不改** —— adapter 启动时自然从项目 `.pi/mcp.json` 读到。v1.1 仅新增显式的当前会话重载路径。
...
2. **direct tools 不随 Pack 动态注册**：direct tools 在 pi-web 进程启动时按全局 `~/.pi/agent/mcp.json` 注册一次。Pack 的 MCP 引用**不会动态注册 direct tools**，只影响 proxy 工具背后的 server 集合 + 实际连接。
   → 库表单和 JSON 导入拒绝 `directTools`；需要它时必须编辑全局 `mcp.json` 并重启 pi-web。

3. **项目 `.mcp.json` 覆盖 Pack 同名 server**：adapter 优先级里 shared-project 高于 pi-project，所以团队的 `.mcp.json` 会盖掉 Pack 写进 `.pi/mcp.json` 的同名条目。项目视图显式提示覆盖关系。
...
6. **argv 全局默认的设置途径**：pi-web 部署时不走 pi CLI argv，全局 direct tools 基线只能是 `~/.pi/agent/mcp.json`。pi-web 仅提供只读 + 文件查看器编辑入口。
...
2. **混合 Pack 的执行原子性**：预览先完成 skill 与 MCP 的全部校验；任何预览冲突均不写文件。执行阶段将新技能先写入临时目录、`.pi/mcp.json` 以原子 rename 替换，项目收据最后提交；任一步失败要还原旧 MCP 文件并删除本次新建技能。这样不会留下“标签已应用但只有半个 Pack”的状态。
...
**v1.0（MCP 包 MVP）**
1. adapter status API + 前后端 Guard；复用现有 package 安装/启用操作。
2. 库扩展 MCP server 文件 `mcp-servers/<key>.mcp.json`（CRUD + 校验 + configHash）。
3. Pack 和工作空间收据升级到 v2（`mcpServers` 引用、`managedServers`、禁用集合），向后兼容迁移。
4. MCP 应用：完整 Pack 并集重整 `.pi/mcp.json` + 预览 + 收据 + 本地冲突保护 + 撤销，默认启用。
5. SkillsConfig Workspace 标签加 MCP 区：列出 + Toggle 启用/禁用（`PATCH /api/workspace-mcp/toggle`）。
6. 项目 MCP 视图右面板（只读 + 合并预览）。
7. Pack 编辑器加 MCP servers 区。

**v1.1**
8. 当前会话 MCP 重载（项目 Pack 已变更后执行 `inner.reload()`，带 UI 确认对话框）。

**v2**
9. OAuth 完整 UI（auth-start/complete SSE）。
10. 从其它 host 导入到库。
11. 连接测试、模板库、侧边栏徽标。
