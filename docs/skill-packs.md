...
**Unapply 有文件删除副作用。** `DELETE /api/workspace-skill-packs?cwd=...&packId=...` 会删除该 Pack 当前定义中不再被其他已应用 Pack 引用的技能目录和 MCP 服务器。MCP 仅在 Pi-web 仍拥有该条目时才会删除；团队配置、预存条目及手工修改过的条目会保留。当 Pack 定义已被删除时，它才回退到 receipt。当前实现也会删除曾因 `same_name_exists` 而跳过的预存技能；不要把 unapply 当作无副作用的“移除标签”。
